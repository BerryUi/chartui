...
class PercentStackedBarChartLayout implements DesignLayout {
   Color netColor;
  ...
  //  Function offRefactor;
   ...
   PercentStackedBarChartLayout({
    ...
    // }
      ...
      //_____________________FOR_SUM_OF_ALL_VALUE________________
    for (var element in x) {
      double summation = 0;
      for (var elementDeep in element) {
        summation+=elementDeep;
      }
      maxfromValue.add(summation);
      // maxLocal =-double.maxFinite;
    }
    ...
    final cXlabelOffset = Offset(labelWidth,top+ 4*hd+10);//Offset(c.dx+20,top+ 4*hd+10);
    ...
    double singleIncrementX = 100/10;

    // double drawableXGap = drawableWidth/10;

    double drawableXGap = (drawableWidth-labelWidth)/10;
    ...
    double barHeightAllotVarX = drawableXGap/singleIncrementX;
      ...
      final axisDrawer =AxisDrawer(
        ...
        maxWidth: labelWidth ,
        xdoubleAxisdata:    maxfromValue,                                 // x,
        yAxisdata:y, 
    ...
    axisDrawer.drawXPercentFixedlabels();
